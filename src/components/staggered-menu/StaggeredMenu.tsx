...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (preContainer) {
        preContainer.querySelectorAll('.sm-prelayer').forEach((el) => preLayers.push(el as HTMLElement))
      }
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (socialTitle || socialLinks.length) {
      ...
      if (socialLinks.length) {
        tl.to(socialLinks as gsap.TweenTarget, {
          y: 0, opacity: 1, duration: 0.55, ease: 'power3.out',
          stagger: 0.08,
          onComplete: () => { gsap.set(socialLinks as gsap.TweenTarget, { clearProps: 'opacity' }) },
        }, socialsStart + 0.04)
      }
    }
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        {logoUrl && (
          <div className="sm-logo" aria-label="Logo">
            <img src={logoUrl} alt="Weminal" className="sm-logo-img" draggable={false} height={32} />
          </div>
        )}
        <button
          ref={toggleBtnRef}
          ...
          onClick={toggleMenu}
          type="button"
          style={{ marginLeft: logoUrl ? undefined : 'auto' }}
        >
