  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ;[j1, j2].forEach((ref) => {
        ...
        const d = Math.max(0.1, Math.min(1, r.lerped.distanceTo(r.translation())))
        r.lerped.lerp(r.translation(), delta * (minSpeed + d * (maxSpeed - minSpeed)))
      })
      ...
      const m = band.current as THREE.Mesh & { geometry: { setPoints: (p: THREE.Vector3[]) => void } }
      if (m) m.geometry.setPoints(curve.getPoints(isMobile ? 16 : 32))
